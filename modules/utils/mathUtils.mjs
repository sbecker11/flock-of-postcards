...
function testArrayFunctions() {
    const arr = [1, 2, 3];
    if ( !isNumericArray(arr) ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: isNumericArray(${arr}) = false but expected true`);
        numErrs++;
    }
    try {
        validateIsNumericArray(arr);
    } catch (e) {
        window.CONSOLE_LOG_IGNORE(`ERROR: validateIsNumericArray(${arr}) threw error but expected no error`);
        numErrs++;
    }
    if ( arrayHasNaNs(arr) ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: arrayHasNaNs(${arr}) = true but expected false`);
        numErrs++;
    }
    const arr2 = [1, 2, 3];
    if ( arraysAreEqual(arr, arr2) ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: arraysAreEqual(${arr}, ${arr2}) = true but expected false`);
        numErrs++;
    }
}
...
function testMinMaxAbs() {
    ...
    if ( abs(-val1) != val1 ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: abs(-${val1}) = ${abs(-val1)} but expected ${val1}`);
        numErrs++;
    }
    if ( abs_diff(val1, val2) != val2 - val1 ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: abs_diff(${val1}, ${val2}) = ${abs_diff(val1, val2)} but expected ${val2 - val1}`);
        numErrs++;
    }
    if ( min(val1, val2) != val1 ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: min(${val1}, ${val2}) = ${min(val1, val2)} but expected ${val1}`);
        numErrs++;
    }
    if ( max(val1, val2) != val2 ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: max(${val1}, ${val2}) = ${max(val1, val2)} but expected ${val2}`);
        numErrs++;
    }
    if ( max3(val1, val2, val3) !== val3 ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: max3(${val1}, ${val2}, ${val3}) = ${max3(val1, val2, val3)} but expected ${val3}`);
        numErrs++;
    }
}
...
function testClamps() {
    ...
    if (result !== expected) {
        window.CONSOLE_LOG_IGNORE(`ERROR: clamp(${value}, ${minVal}, ${maxVal}) = ${result} but expected ${expected}`);
        numErrs++;
    }
    const value2 = 2.5;
    const expected2 = 2;
    ...
    if (result2 !== expected2) {
        window.CONSOLE_LOG_IGNORE(`ERROR: clampInt(${value2}, ${minVal}, ${maxVal}) = ${result2} but expected ${expected2}`);
        numErrs++;
    }
}
...
function testLinearInterp() {
    ...
    if (result !== expected) {
        window.CONSOLE_LOG_IGNORE(`ERROR: linearInterp(${x}, ${x0}, ${y0}, ${x1}, ${y1}) = ${result} but expected ${expected}`);
        numErrs++;
    }
}
...
function testArrayOfArrays() {
    ...
    if (!validateIsArrayOfArrays(arr) ) {
        window.CONSOLE_LOG_IGNORE(`ERROR: validateIsArrayOfArrays(${arr}) threw error but expected no error`);
        numErrs++;
    }
}
...
function testDistances() {
    ...
    if (result !== expected) {
        window.CONSOLE_LOG_IGNORE(`ERROR: getPositionsEuclideanDistance(${pos1}, ${pos2}) = ${result} but expected ${expected}`);
        numErrs++;
    }
    if ( getPositionsSquaredDistance(pos1, pos2) !== 2) {
        window.CONSOLE_LOG_IGNORE(`ERROR: getPositionsSquaredDistance(${pos1}, ${pos2}) = ${result} but expected ${expected}`);
        numErrs++;
    }
    ...
    if (result !== expected) {
        window.CONSOLE_LOG_IGNORE(`ERROR: getEuclideanDistance(${arr1}, ${arr2}) = ${result} but expected ${expected}`);
        numErrs++;
    }

    if ( getSquaredDistance(arr1, arr2) !== 2) {
        window.CONSOLE_LOG_IGNORE(`ERROR: getSquaredDistance(${arr1}, ${arr2}) = ${result} but expected ${expected}`);
        numErrs++;
    }
}
...
function testRandomOffset() {
    ...
    if (result < -maxOffset || result > maxOffset) {
        window.CONSOLE_LOG_IGNORE(`ERROR: getRandomSignedOffset(${maxOffset}) = ${result} but expected a value between -${maxOffset} and ${maxOffset}`);
        numErrs++;
    }
}
...
export function test_mathutils() {
    ...
    testRandomOffset();
    if ( numErrs !== 0 ) {
        window.CONSOLE_LOG_IGNORE(`mathUtils: ${numErrs} tests failed`);
    }
}