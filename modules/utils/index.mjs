...
import * as css_colors from '../colors/css.mjs';
import { Logger, LogLevel } from '../logger.mjs';
import { logger } from "../logger.mjs";

// DOM Elements
let _focalPointElement = null;
let _aimPointDotElement = null;
...
let _isBeingDragged = false;
let _isLockedToBullsEye = false;
let _isEasingToAimPoint = false;
let _isEasingToBullsEye = false;
let _mouseDrag = null;

// Constants
const EPSILON = 0.1;
const EASE_FACTOR = 0.1;
const MAX_NEAR_DISTANCE = 50;
...
export function showElement(element, prefix="", logLevel=LogLevel.LOG) {
    ...
    if (isCardDiv(element)) {
        const nextSibling = this.findNextSiblingWithClass(element, "card-div");
        nextSiblingId = (nextSibling != null) ? nextSibling.id : "";
    } else if (isBizcardDiv(element)) {
        const nextSibling = this.findNextSiblingWithClass(element, "bizcard-div");
        nextSiblingId = (nextSibling != null) ? nextSibling.id : "";
    }
    const center = {
