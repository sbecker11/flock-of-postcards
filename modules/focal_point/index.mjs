...
export function initialize() {
    ...
    // Initialize focal point position if saved
    if (state.lastPosition) {
        moveFocalPointTo(state.lastPosition);
    }
    ...
    if (!_resizeObserver) {
        _resizeObserver = new ResizeObserver(() => {
            ...
            if (_isLockedToBullsEye) {
                startEasingToBullsEye("ResizeObserver");
            }
        });
    ...
    _isDraggable = loadDraggableState();

    logger.log("Focal point initialized with state:", state);
    return state;
}
...
export function moveFocalPointTo(position, prefix="") {
    if (!_focalPointElement) {
        logger.error("Cannot move focal point: element not found");
        return;
    }
    ...
    // // Calculate the center offset based on the focal point's dimensions
    // const rect = _focalPointElement.getBoundingClientRect();
    // const centerOffsetX = rect.width / 2;
    // const centerOffsetY = rect.height / 2;

    // // Position the focal point so its center is at the target position
    // _focalPointElement.style.left = `${position.x - centerOffsetX}px`;
    // _focalPointElement.style.top = `${position.y - centerOffsetY}px`;
...
export function getFocalPointElement() {
    if ( _focalPointElement == null )
        return null;
    return _focalPointElement;
}

export function getAimPointDotElement() {
...
// export function setAimPoint(position, prefix="") {
//     _aimPoint = position;
//     if (_aimPointDotElement) {
//         _aimPointDotElement.style.left = `${position.x}px`;
//         _aimPointDotElement.style.top = `${position.y}px`;
//     }
//     logger.log(prefix, "setAimPoint:", _aimPoint);
// // }

// export function startEasingToAimPoint(prefix="") {
//     if (!_aimPoint) {
//         logger.error("Cannot ease to aim point: no aim point set");
//         return;
//     }
//     _isEasingToAimPoint = true;
//     _isEasingToBullsEye = false;
//     setStatus("easing to aim point", prefix);
// // }

// export function startEasingToBullsEye(prefix="") {
//     _isEasingToAimPoint = false;
//     _isEasingToBullsEye = true;
//     setStatus("easing to bulls eye", prefix);
// }

export function stopEasing(prefix="") {
    _isEasingToAimPoint = false;
    _isEasingToBullsEye = false;
    setStatus("stopped easing", prefix);
}


export function updateFocalPoint() {
    if (!_focalPointElement || !_isAwake) {
        return;
    }

    let targetPoint;
    if (_isEasingToBullsEye) {
        targetPoint = getBullsEye();
    } else if (_isEasingToAimPoint && _aimPoint) {
        targetPoint = _aimPoint;
    } else {
        return;
    }

    const currentPoint = getFocalPoint();
    ...
    // If we're close enough to the target, stop easing
    if (distance < EPSILON) {
        if (_isEasingToBullsEye) {
            _isEasingToBullsEye = false;
            setStatus("reached bulls eye");
        } else if (_isEasingToAimPoint) {
            _isEasingToAimPoint = false;
            setStatus("reached aim point");
        }
        return;
    }

    // Calculate new position with easing
    const newX = currentPoint.x + (dx * EASE_FACTOR);
    const newY = currentPoint.y + (dy * EASE_FACTOR);

    // Move focal point to new position
    moveFocalPointTo({ x: newX, y: newY });
}

export function isNearBullsEye() {
    const focalPoint = getFocalPoint();
    const bullsEye = getBullsEye();
    
    if (!focalPoint || !bullsEye) {
        return false;
    }

    const dx = bullsEye.x - focalPoint.x;
    const dy = bullsEye.y - focalPoint.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
...
export function toggleLockToBullsEye() {
    _isLockedToBullsEye = !_isLockedToBullsEye;
    if (_isLockedToBullsEye) {
        startEasingToBullsEye("toggleLockToBullsEye");
    }
    saveState();  // Save the new lock state
