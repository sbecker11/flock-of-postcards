...
export function test_HSV_RGB_Hex_functions() {
    function test_get_HSV_from_RGB() {
        ...
        if ( rgbDist > EPSILON )
            CONSOLE_LOG_IGNORE(`ERROR: rgbDist:${rgbDist} exceeds EPSILON:${EPSILON}`);
    }
    function test_get_RGB_from_HSV() {
        ...
        if (hsvDist > EPSILON ) 
            CONSOLE_LOG_IGNORE(`ERROR: hsvDist:${hsvDist} exceeds EPSILON:${EPSILON}`);
    }
    function test_get_HSV_from_Hex() {
        ...
        if ( hexDiff > 1 ) {
            CONSOLE_LOG_IGNORE(`ERROR: hexDiff:${hexDiff} exceeds 1`);
        }
    }
...
export function test_RGB_RgbStr_functions() {
    function test_get_RGB_from_RgbStr() {
        ...
        if ( rgbStrIn !== rgbStrOut )
            CONSOLE_LOG_IGNORE(`ERROR: rgbStrIn:${rgbStrIn} != rgbStrOut:${rgbStrOut}`);
    }
    function test_get_RgbStr_from_RGB() {
        ...
        if ( !mathUtils.arraysAreEqual(rgbOut, rgbIn) )
            CONSOLE_LOG_IGNORE(`ERROR: rgbOut:${rgbOut} != rgbIn:${rgbIn}`);
    }
...
export function test_RGB_ColorStr_functions() {
    ...
    function test_get_RGB_from_ColorStr() {
        ...
        if ( ColorStrOut != ColorStrIn )
            CONSOLE_LOG_IGNORE(`ERROR: ColorStrOut:${ColorStrOut} != ColorStrIn:${ColorStrIn}`);
    }
...
export function test_getHighContrastCssHexColorStr() {
    ...
    CONSOLE_LOG_IGNORE(`lo_RGB:${lo_RGB} lo_Hex:${lo_Hex} -> hi_Hex:${hi_Hex} hi_RGB:${hi_RGB}`);
    if ( hi_RGB[0] !== 0xff || hi_RGB[1] !== 0xff || hi_RGB[2] !== 0xff ) {
        CONSOLE_LOG_IGNORE("FAILURE expected white, not hi_RGB:", hi_RGB);
    } 
    ...
    CONSOLE_LOG_IGNORE(`hi_Hex:${hi_Hex} hi_RGB:${hi_RGB} -> lo_RGB:${lo_RGB} lo_Hex:${lo_Hex} `);
    if ( lo_RGB[0] !== 0 || lo_RGB[1] !== 0 || lo_RGB[2] !== 0 ) {
        CONSOLE_LOG_IGNORE("FAILURE expected black, not lo_RGB:", lo_RGB);
    } 
}
...
export function test_colorUtils() {
    ...
    if ( rgbDiff > 1.0 ) {
        CONSOLE_LOG_IGNORE(`ERROR: rgbDiff:${rgbDiff} exceeds 1.0`);
    }
}