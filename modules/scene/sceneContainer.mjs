...
export function updateSceneContainer() {
    // CONSOLE_LOG_IGNORE("updateSceneContainer");
...
export function getSceneVerticalPositionForDateString(dateStr) {
    ...
    CONSOLE_LOG_IGNORE(`******* yearString:${yearString} monthString:${monthString} -> position:${position}`);

    // Add debug logging
    //CONSOLE_LOG_IGNORE(`Date: ${dateStr} -> Year: ${yearString}, Month: ${monthString} -> Position: ${position}`);
...
export function getSceneVerticalPositions(startDateStr, endDateStr, minHeight=0) {
    ...
    if (pixelDiff > pixelsPerMonth) {
        ...
        CONSOLE_LOG_IGNORE(`Date range pixel match: startDate:${startDate} endDate:${endDate} - Expected:${expectedPixels}px, Actual:${sceneHeight}px, Diff:${pixelDiff}px`);
    }
...
export function ensurePointerEvents() {
    ...
    if (sceneContainer.style.pointerEvents !== 'auto') {
        CONSOLE_LOG_IGNORE("Fixing scene container pointer-events");
    ...
    bizCardDivs.forEach(div => {
        if (div.style.pointerEvents !== 'auto') {
            CONSOLE_LOG_IGNORE(`Fixing pointer-events for ${div.id}`);
            div.style.pointerEvents = 'auto';
        }
    });
    
    CONSOLE_LOG_IGNORE("Scene container and bizCardDivs pointer events fixed");
}
...
export function updateGradientHeights() {
    ...
    CONSOLE_LOG_IGNORE(`Updated scene plane height: ${timelineHeight}px, gradient height: ${gradientHeight}px`);
}
