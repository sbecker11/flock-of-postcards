...
import { get_z_from_z_index, MIN_BIZCARD_Z_INDEX } from '../layout/zIndex.mjs';
...
function positionBizCardDiv(bizCardDiv, startDate, endDate) {
    ...
    const widthOffset = getRandomOffset(MAX_WIDTH_OFFSET);
    const xOffset = getRandomOffset(MAX_X_OFFSET);
    
    // Get viewport dimensions
    const viewport = getViewport();
    
    // Calculate card dimensions
    const cardWidth = BIZCARD_WIDTH + widthOffset;
    
    // Scene geometry:
    // - Bullseye is at midpoint between window left and divider
    // - Cards should be distributed around bullseye
    const cardCenterX = viewport.bullseyeX + xOffset;
    
    // Calculate card left edge position
    // left = center - width/2
    ...
    bizCardDiv.style.width = `${cardWidth}px`;
...
export function createBizCardDiv(job, jobIndex, canvas) {
    ...
    const z_index = MIN_BIZCARD_Z_INDEX + jobIndex;
