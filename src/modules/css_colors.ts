...
interface CssColorsMap {
  [key:string]: HexColor;
}

const LOWERCASE_CSS_COLORS = _getLowerCaseCssColors();

function _getLowerCaseCssColors(): CssColorsMap  {
    const lowerCaseColors: CssColorsMap = {};
...
export function get_HEX_from_CssColor(
    cssColor: string,
    verbose: boolean = false 
): HexColor | null {
    if ( utils.isString(cssColor) ) {
        const color = cssColor.toLowerCase();
        const colors = LOWERCASE_CSS_COLORS;
        if ( color in colors ) {
...
export function get_CssColor_from_HEX(
    HEX: string, 
    verbose: boolean = false
): CssColorName | null {
    if ( utils.isHexColorString(HEX) ) {
