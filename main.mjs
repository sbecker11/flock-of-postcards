...
import * as cards from './modules/cards/legacy/index.mjs';
import * as utils from './modules/utils/index.mjs';
import * as timeline from './modules/timeline/index.mjs';
import * as focalPoint from './modules/focal_point/index.mjs';
import { getPaletteSelectorInstance } from './modules/colors/palettes.mjs';
import { logger } from "./modules/logger.mjs";
import { 
    // ... existing imports ...
    handleKeyDown as handleFocalPointKeyDown,
} from './modules/focal_point/index.mjs';
// import { cardManager } from './modules/cards/modern/manager.mjs';
// import { create_BizCardDivs } from './modules/cards/modern/manager.mjs';
...
function selectTheCardDiv(cardDiv) {
    ...
    // Validate card type
    if ((!isCardDivId(cardDiv.id) && !isBizcardDivId(cardDiv.id))) {
...
async function createAllElements() { // Add async
    ...
    // Initialize old card system
    // createBizcardDivs();  // Commented out old card system
    // addAllIconClickListeners();  // Commented out old card system

    // Initialize new card system in parallel
    // This will create cards with -card-div- class names that won't conflict with old system
    cardManager.initialize(jobs);
...
function updateBullsEyePosition() {
    ...
    const bullsEye = document.getElementById('bulls-eye');
    if (bullsEye) {
        // Calculate center position considering the transform
        const centerX = canvasRect.left + (canvasRect.width / 2);
        const centerY = canvasRect.top + (canvasRect.height / 2);
        
        // Set position - the transform: translate(-50%, -50%) will center it
        bullsEye.style.left = `${centerX}px`;
        bullsEye.style.top = `${centerY}px`;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        // Ensure jobs array is available (should be loaded from jobs.mjs in index.html)
        if (typeof jobs === 'undefined') {
            throw new Error('Jobs data not found. Make sure jobs.mjs is loaded before main.mjs');
        }

        // Initialize the legacy card system
        cards.initialize(jobs);
        cards.positionCards();

        // Add cards to canvas
        const canvas = document.getElementById('canvas');
        cards.bizCards.forEach(card => canvas.appendChild(card.element));

        logger.log("Legacy card system initialized with:", {
            bizCards: cards.bizCards.size
        });
...
window.addEventListener('load', async () => {
    ...
    focalPoint.initialize(focalPointElement, canvasContainer);
    
    // Initialize palette selector
    paletteSelector = getPaletteSelectorInstance();
});